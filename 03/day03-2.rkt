...
(define (parse-input in_p)
  (let loop ((cur (read-char in_p))
    ...
    (cond ((and
            ...
            (not (null? cur-num)))
           (loop cur col row '() '() sym-coords (cons (cons (string->number (apply string cur-num)) (list->vector cur-num-coords)) num-acc)))
          ((eof-object? cur)
           (values (list->vector num-acc) (list->vector sym-coords)))
          ((char=? cur #\newline)
           (loop (read-char in_p) 0 (add1 row) '() '() sym-coords num-acc))
          ((char=? cur #\*)
           (loop (read-char in_p) (add1 col) row '() '() (append sym-coords (list (cons row col))) num-acc))
          ((char-numeric? cur)
           (loop (read-char in_p) (add1 col) row (append cur-num (list cur)) (append cur-num-coords (list (cons row col))) sym-coords num-acc))
          (else (loop (read-char in_p) (add1 col) row '() '() sym-coords num-acc)))))
